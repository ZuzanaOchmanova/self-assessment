...
/** Quick wins by stage */
export const QUICK_IMPROVEMENTS_BY_STAGE: Record<number, string> = {
  0: "Choose one high-value system—such as your CRM—and automate a nightly export into a single shared folder or simple database table. Even this one “single source of truth” will immediately cut down on version conflicts and give your team confidence in the data they see.",
  1: "Automate a simple health check on your existing imports—have each nightly or weekly job verify key row counts or schemas and email you if something fails. This ensures you catch basic issues before they slip into your spreadsheets.",
  2: "Convert one of your bespoke data-load scripts into a reusable template that runs on a schedule. By parameterizing source details, you can add new systems in minutes instead of days, and ensure your dashboards always draw from the same up-to-date data.",
  3: "Add a simple end-to-end health check for one of your core pipelines: have the process verify that key tables contain the expected row counts or timestamp freshness and send a notification if anything falls outside the norm. This one small step turns blind spots into proactive alerts, catching failures before they affect your dashboards.",
  4: "Turn on real-time notifications for your existing checks by routing audit failures and data anomalies into a dedicated chat channel or paging service. That way, incidents surface immediately and prompt the right team members to investigate before dashboards are impacted.",
  5: "Wire your existing alerts into an escalation workflow: have any unacknowledged failure or data anomaly automatically notify a backup team or paging service after a brief delay, ensuring every incident gets attention without human prodding.",
  6: "Embed a simple machine-learning anomaly detector into one of your key data flows—use an open-source library or managed service to learn normal patterns and flag outliers in real time, sharpening your proactive monitoring."
};

/** Long-term goals by stage */
export const LONG_TERM_GOALS_BY_STAGE: Record<number, string> = {
  0: "Build a fully modular data platform that ingests all core systems on schedule, enforces basic quality checks and deduplication at ingest, powers live self-service dashboards and alerts, and layers on access controls and policy enforcement. Over time, evolve this foundation into a real-time, AI-ready environment that delivers trustworthy, automated insights at scale.",
  1: "Consolidate your data into a true shared repository by moving all core system exports into a lightweight database or cloud table. Pair this with parameterized dashboards that refresh automatically and pull from that single source—so your team stops juggling files and begins building trust in a consistent, up-to-date view of the business.",
  2: "Build a fully managed data pipeline framework that sequences all your imports, applies automated quality and deduplication checks at each step, and writes into a dimensional model in your warehouse. Layer on self-service dashboards powered by that clean, centralized data so every team accesses live, trustworthy insights without manual intervention.",
  3: "Build an orchestrated, policy-driven data platform where every pipeline runs under a central scheduler, carries built-in quality and lineage checks, enforces ownership and access controls at every step, and feeds real-time dashboards and alerts. By embedding governance and monitoring directly into each data flow, you’ll achieve truly hands-off reliability at scale—and lay the groundwork for AI-powered insights down the road.",
  4: "Evolve into a fully automated, policy-driven platform where every data pipeline, quality rule, and access policy are codified and enforced at runtime. Embed dynamic, attribute-based controls so permissions adjust on the fly, and integrate automated remediation workflows that quarantine or correct bad data as soon as it’s detected—ultimately delivering handsoff reliability and trust at scale.",
  5: "Translate your governance and monitoring rules into “policy-as-code” so that every data pipeline enforces ownership, access, quality checks, lineage tracking, and remediation logic automatically. Embed these policies into your deployment process so that any breachtriggers corrective actions—quarantine, retries, or rollback—without operator intervention, delivering truly hands-off reliability at scale.",
  6: "Develop a fully autonomous data ecosystem where governance, quality, and feature engineering are all codified as executable policies: every data pipeline enforces rules at runtime, automatically corrects or quarantines bad data, and exposes versioned features to downstream applications via APIs—realizing a true “set-and-forget” foundation for continuous AI innovation."
};